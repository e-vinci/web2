...
Pour continuer notre tutorial (dans le répertoire **`structured`**), veuillez compléter le code de **`/src/Component/Pages/Login.js`** :
...
Quand la navigation fonctionne correctement pour naviguer entre la **`HomePage`** et **`AboutPage`** à l'aide de votre **`Navbar`**, veuillez faire un **`commit`** de votre code avec comme message : **`2.12 : dynamic page content`**.
