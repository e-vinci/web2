...
Veuillez démarrer l'API qui n'autorise aucune autre origine : **`/web2/tutorials/pizzeria/api/fat-model`** (ou via le code de ce web repo si vous avez un souci : [fat-model](https://github.com/e-vinci/js-demos/tree/main/backend-restful-api/restful-api-essentials/fat-model)).
...
Pour ce nouveau tutoriel, au sein de votre repo **`web2`**, veuillez créer le projet nommé **`/web2/tutorials/pizzeria/hmi/basic-fetch`** sur base d'un copié/collé de **`/web2/tutorials/pizzeria/hmi/basic-fetch-no-proxy`** (ou [basic-fetch-no-proxy](https://github.com/e-vinci/js-demos/tree/main/frontend/frontend-essentials/basic-fetch-no-proxy)).
...
Si tout fonctionne bien, faites un **`commit`** de votre repo (**`web2`**) avec comme message :
**`basic-fetch with proxy tutorial`**.
...
Veuillez enrichier la SPA développée dans le cadre de votre projet en lui ajoutant :
...
- vous pouvez directement utiliser votre API existante développée pour l'[Exercice 1.10](../../part1/refactoring/#exercice_1_10_encore_un_fat_model).
