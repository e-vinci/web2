...
Actuellement, si vous êtes étudiant, vous pouvez bénéficier de 100$ de crédit et vous n'avez pas besoin de carte de crédit ! En suivant le tutoriel proposé par Microsoft, vous vous rendrez sur [free with Azure for Student](https://azure.microsoft.com/en-in/free/students/) et utiliserez par la suite **Azure App Service**. 
...
#### 🤝 Tips
Il est possible que votre déploiement soit facilité si vous créez un repository pour votre frontend et un repository pour votre backend : la racine de vos repositories correspondront à la racine de vos projets ; pour vous en assurer, à la racine de chaque repo, se trouvera un fichier `/packages.json`.
