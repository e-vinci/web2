...
- Finalement, si l'opération de création de la pizza réussi, nous redirigeons l'utilisateur vers la **HomePage** à l'aide du composant fonctionnel **`Navigate.js`** offert au sein du fichier **Navigate** dans le dossier **Router** du boilerplate du frontend.
...
Il est si simple, à l'aide d'**async** / **await**, de créer des fonctions asynchrones qui renvoient une promesse, qu'il serait dommage de ne pas l'expérimenter.
...
# <a id="e"></a> e) Exercice 6.1 : Consommation d'une API tierce

Nous souhaitons consommer une API qui nous permette d'afficher des paroles célèbres d'Anime.  
Nous avons trouvé une restful API qui permet très facilement de générer de manière aléatoire des **quotes** : [Animechan](https://animechan.vercel.app/).

Cette API est très simple d'utilisation. D'ailleurs, la page du site [Animechan](https://animechan.vercel.app/) vous donne un exemple fonctionnel de comment récupérer une **quote**.

Dans la **HomePage** d'une nouvelle application frontend, veuillez afficher une **quote** après l'avoir récupérée de l'API Animechan, en donnant ces 3 informations :
- le nom de l'Anime associé à la **quote** ;
- le personnage associé à la **quote** ;
- le texte de la **quote**.
...
Veuillez faire un **commit** de votre code avec le message suivant : "**module 6 : exercise 6.1 read from animeChan**".

# <a id="f"></a> f) Exercice 6.2 : Intégration de la RESTful API au sein de l'IHM de myMovies via un proxy
...
# <a id="g"></a> g) 🍬 Exercice 6.3 : Autoriser des nouvelles origines via la RESTful API
