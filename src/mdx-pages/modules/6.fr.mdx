...
formulaire est envoyé au serveur, l'action du formulaire est de faire l'appel à une nouvelle page
au serveur (via la propriété "action" du formulaire HTML) ; jusqu'à la réponse du backend, aucune
...
sont une technologie de communication temps-réel client / serveur et bidirectionnelle.  
A la fin de ce cours, vous devriez être apte à découvrir cette technologie par vous même si vous
le souhaitiez.

Avec **AJAX**, c'est le client qui doit initier la communication. Ca n'est pas le cas pour les
**websockets**, le serveur peut le faire. Ainsi, avec **AJAX**, le client doit créer une connexion HTTP à chaque requête.
...
Ancienement, il y a environ 20 ans, c'est la librairie **XMLHttpRequest** qui était utilisée.
...
Hé bien c'est ça la programmation asynchrone, ça n'est qu'une fois le programme principal exécuté que les tâches asynchrones, de priorité plus basses, pourront s'executer.
...
**CORS** est un méchanisme qui utilise des headers HTTP pour indiquer aux browsers qu'ils peuvent autoriser les accès à des ressources d'origines différentes.
...
Les **CORS** permettent de relacher la sécurité, afin notamment, dans le scénario évoqué, d'autoriser l'origine `https://my-bank.com` à accéder à l'origine `https://api.my-bank.com`. En effet, c'est ce que le site de la banque souhaite.  
...
- **await** est utilisé pour chaîner une tâche asynchrone (sur une fonction renvoyant une promesse) et ne peut se faire qu'au sein d'une fonction taggée par **async** ; c'est donc le remplaçant du **.then(callback)**.  
...
- Toute fonction "taggée" par **async** renvoie automatiquement une promesse ; cela signifie dans le code ci-dessus que la fonction **HomePage** est elle même asynchrone.
