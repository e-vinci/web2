...
Si vous exécutez l'application, elle lancera des exceptions lorsque vous appelerez certaines 
opérations.
...
Afin d’injecter des dépendances au sein d’une application implémentant Jackarta EE, 
...
```
- Pour rappel, l’annotation **@Provider** ci-desssus permet de fournir notre propre implémentation 
...
qui doit attrapper toutes les exceptions, afficher la stack trace, puis envoyer une réponse 
...
## c.2) Filtrer des attributs de la représentation JSON d'un POJO
...
Le mécanisme qui a été sélectionné pour ce cours est la création de vues JSON au sein d’un POJO.
<br/>

Nous allons créer deux vues JSON que nous pourrons associer au POJO **User** implémenté 
par **UserImpl** : 
...
de la vue publique sont initialisés à null. 
<br/>
...
Cette méthode va mettre à null tous les champs non publics (ici juste le password) d'un User et 
...
est à null en JSON, sans sérialiser le champs **password** car nous avons annoté 
(**@JsonInclude(JsonInclude.Include.NON_NULL)**) **UserImpl** pour spécifier que seulement 
les attributs non nuls seront inclus dans la sérialisation.
...
authentifié pour les news dont le statut vaut « published ».
Pour les news dont le statut vaut « hidden », seul leur auteur pourra les lire.
<br/>
...
Tous les news dont le statut vaut « published » pourront être lues. 
...
Quelques tips :
- Continuer le projet développé dans le cadre de l'Exercice 2.1 du module 2. Il n'est pas nécessaire 
...
Challenges optionnels : Nous vous proposons d’utiliser les vues JSON afin de ne jamais écrire 
dans **db.json** toutes les infos liées à la page associée à une news. 
...
Finalement, n’hésitez pas à faire un refactoring du code pour tout ce qui est associé 
aux ressources de type “Page”.
<br/>
