...
- h) [Gesion des caractères dangereux](/modules/1#sectionH)
...
devra contenir toutes les données pour être authentifié / authorisé.
<br/>

Une RESTful API met à disposition des pérations sur des ressources via :
- des URI
- des méthodes HTTPS
...
# <a id="sectionB"></a>b) Creation d’une première application sur base d’un Archetype Maven
...
A l’aide d’IntelliJ, créez un projet Maven en utilisant un archetype fournit par Jersey:
-	Clic sur “Create from archetype” à la création de votre projet
-	Sélectionnez dans la liste des archetypes : **org.glassfish.jersey.archetypes :jersey-quickstart-grizzly**

<br/>

Explorez ce qui est offert au sein de ce projet :
- Lancez la méthode **main** de la classe **Main**.
- Une tout première méthode **getIt** est offerte au sein de la ressource **MyResource**.
- Consommez cette méthode **getIt**  à l’aide d’un client, via un browser, à l’URL [http://localhost:8080/myapp/myresource](http://localhost:8080/myapp/myresource) 
...
au sein de l’application. Le framework pourra injecter des instances de ces resources le temps voulu.
<br/>
...
et spécifiez que la représentation des resources renvoyées par votre méthode **getAll** est du JSON :
```java
@GET
...
ne dépend pas de l'état de la resource, il est inutile de créer une nouvelle instance de ressource pour chaque requête.
<br/>
...
Veuillez noter comment un message de status est renvoyé au client si aucune resource n'est trouvée.
...
Dès lors, la réponse contiendra le code de status **404** (équivalent de **NOT_FOUND**) et le message 
"Resource not found".
<br/>
...
Plus d'info sur la désérialisation via Jackson : 
...
Afin de permettre la désérialisation par le framework, on doit préciser le type **MIME** de la représentation de la resource qui est envoyée dans 
...
## f.2) REST Client sous VS Code
Dans le cadre de ce cours, nous pouvons utiliser REST Client de Visual Studio Code : 
...
- Nous allons charger toutes les ressources se trouvont dans un fichier JSON au sein d'un objet Java, 
...
Pour rendre les ressources peristantes au sein de notre API de gestion de films, veuillez créer 
une nouvelle classe **Json**.
