...
Un mécanisme d'authenfication **stateful** indique que le serveur est responsable de sauvegarder les données de session des utilisateurs. 

On parle de **session** d'un utilisateur commme étant toute la durée où le client s'authentifie à l'application web jusqu'à ce qu'il quitte cette application, lorsqu'il ferme son browser.
...
Il existe différents moyen de l'implémenter. Dans le cadre de ce cours, nous focuserons sur la façon la plus habituel, les **JSON Web Token**, ou **JWT**.

Il existe d'autres méchanismes très modernes qui utilisent des tokens, mis en place par des tiers : **OAuth**, **OpenId**... Ces méchanismes sont mis en place par Microsoft, Google, Facebook... Nous ne les verrons pas dans le cadre de cours. Néanmoins, à la fin de ce cours, vous devriez être aptes à pouvoir les utiliser, sous réserve de bien lire la documentation.
...
Certaines applications web ont des centaines de millions d'utilisateurs ; pour celles-ci, on ne peut pas compter sur la "scalability" veriticale, c'est-à-dire le fait d'augmenter les ressources d'une machine, en augmentant sa RAM, son processeur, son espace de stockage...
...
Ainsi, on peut imaginer un token comme une enveloppe sécurisé par un cadena très spécial : ce cadenas, qui est mis sur l'enveloppe contenant, par exemple, l'identifiant d'un utilisateur, est fermé par la clé 🔑 que seulement connaît l'API.
Si quelqu'un touche à l'enveloppe, le cadena ne s'ouvrira plus avec la 🔑 de l'API : la tentative de hackage sera détectée ! 
Si quelqu'un tente de créer une enveloppe sécurisé, il le fera avec une autre clé, **🗝** par exemple, car il ne connaît pas 🔑. L'API, tentant d'ouvrir l'enveloppe sécurisée à l'aide de 🔑, elle ne s'ouvrira pas : la tentative de hackage sera détectée ! 

💭 Est-ce que cette méthaphore d'enveloppe sécurisée vous parle ?
...
### a.3.3) Mécanisme d'autorisaton et vérification du token
...
Veuillez exécuter le fronted et vous assurer que l'utilisateur que vous tentez de créer est bien créé par votre API. Si tout fonctionne, vous aurez une confirmation dans la console de votre browser.
...
Attention, même si nous avons mis à jour la **Navbar** et créé le composant **Logout**, le boilerplate du frontend impose de rajouter une route au sein du **Router** pour qu'un clic sur l'élément **Logout** de la **Navbar** ammène à appeler le composant **Logout**.
...
La raison est simple, l'API est développée indépendemment des applications clientes, elle ne peut pas supposer que les requêtes seront toujours bien construites.
...
- UC6 : un utilisateur doit pouvoir se logguer (login).
- UC7 : un utilisateur doit pouvoir se délogguer (logout).
...
Veuillez vous logger à l'aide du compte **admin** (et le password **admin**).  
...
Maintenant, tout utilisateur connecté ou pas qui se loggera sur le site va exécuter ce script "malicieux". Il verra le pop-up appraître avec le message "You have been hacked !".  
...
Par rapport à l'attaque précédente, le plus simple pour protéger de l'attaque pourrait être de faire en sorte que l'application cliente n'interprête pas d'HTML / JS / CSS envoyé par l'API.

Ici, dans la **HomePage**, en fait on utilise la propriété **`innerHTML`** d'une **`td`**. Ainsi, l'HTML et l'inline JS associé est exécuté. Si l'on utilisais la propriété **`innerText`** des **`td`**, alors, ni l'HTML et le JS serait interprêté par le browser, et donc pas de possibilité de script malicieux au niveau de la **HomePage**.
...
Nous allons à présent mettre à jour le modèle de pizzas pour echapper les caractères dangereux.
...
Nous avons simplement chaîné les réponses à faire au client seulement une fois les opération `register` et `login` terminéés. Pour ce faire, nous avons précédé le nom de ces méthodes par **await**. Nous avons donc du ajouter **async** aux fonctions middleware s'occupant des routes **POST /auths/register** et **POST /auths/login**.
...
Commencez par hacker votre site en faisant une injection de Javascript. Ce script "malicieux" affichera un pop-up à chaque utilisateur affichant la liste des films ; ce pop-up sera suffisamment explicite pour que vos utilisateurs sachent qu'ils ont été hacké.
