...
Comment gérer la mise à jour des données d'un film ?
  - Option 1 (la plus cool à découvrir 😉) : Vous pourriez faire en sorte que l'HTML du tableau affichant les films, lors d'un clic, devienne éditable. Pour cette option :
    ...
    - Veuillez noter que contrairement au frontend précédent réalisée à l'Exercice 3.1, l'info **title** et **link** ne sont plus intégrées dans une même colonne, via des hyperlinks ; avec l'option 1, nous devrions considérer deux colonnes, une pour le titre et l'autre pour le lien.
