...
Est-ce que ça n'est pas une donnée de session qui doit persister lorsque l'utilisateur ferme son browser ?  
...
En effet, Heroku utilise un système de fichiers éphémère, à chaque fois qu'une dyno est redémarrée, elle utilise une copie du déploiement le plus récent, c'est-à-dire les fichiers qui ont été "push" sur heroku pour lancer le build.
...
## <a id="c4"></a> c.4) Déploiement d'une API sur Azure
...
Attention de choisir la formule gratuite lorsque vous suivrez la procédure, en indiquant le tarif **F1**. Vous aurez le droit à 60 minutes de processing gratuites par jour, après quoi votre app renvoie un status code 403. Votre app n'est pas toujours active, mais il est pas évident d'estimer si vous tiendrez toute une journée sans erreur 403.

Actuellement, si vous êtes étudiant, vous pouvez bénéficier de 100$ de crédit et vous n'avez pas besoin de carte de crédit ! En suivant le tutoriel proposé par Microsoft, vous vous rendrez sur [free with Azure for Student](https://azure.microsoft.com/en-in/free/students/). **Azure App Service**. 
...
## <a id="c5"></a> c.5) Déploiement d'une API sur d'autres providers gratuits
...
## <a id="c6"></a> c.6) Déploiement d'une base de données
### c.6.1) Bases de données relationnelles
...
⚡ Attention, Git ne traque pas les repos dans les repos. Ainsi, vos deux projets ne peuvent pas se trouver dans le repos **web2** de votre cours !  
N'oubliez pas d'inclure un fichier **.gitignore** pour éviter de mettre en configuration **node_modules** !

Nous vous recommandons de suivre la documentation pour déployer une App à l'aide de VS Code, en installant l'extension **Azure App Service**  : [Create a Node.js web app in Azure](https://docs.microsoft.com/en-us/azure/app-service/quickstart-nodejs?tabs=linux&pivots=development-environment-vscode).
...
Vous trouverez 5 images correspondant à chaque lieu dans le répertoire **/question1/src/img**, ainsi qu'un fichier contenant une liste des lieux avec leur id, leur nom et leur image dans **/question1/src/utils/places.js**.
